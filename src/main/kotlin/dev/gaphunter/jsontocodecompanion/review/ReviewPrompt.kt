package dev.gaphunter.jsontocodecompanion.review
 ...
 * used a real number of successful generations -- never on install,
 * never on a timer. Every call to [recordHit] here already corresponds
 * to one real file written (never the "already exists" or validation-
 * failed branches). Counts invocations directly instead of tracking a
 * set of seen keys.
 *
...
object ReviewPrompt {
    ...
    private const val KEY_HIT_COUNT = "dev.gaphunter.jsontocodecompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.jsontocodecompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "JSON to Code Companion"

    // Marketplace listing (real ID resolved after first publish).
    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33550-json-to-code-companion/reviews"

    /**
     * Call this once per real, successful generation. Safe to call on
     * any thread.
     */
